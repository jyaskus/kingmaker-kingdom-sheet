...
$.kingdom.Kingdom = Class.create(
{
	...
	// rivers
	$('[name="rivers"]').setSelect(size);
	var rivers = parseInt(this.getChoice("rivers", 0));
	if (rivers > size)
	{
	    this.setChoice("rivers", size);
	    ribers = size;
	}
	// 
	tRoads = roads + rivers;
	this.modify("Economy", parseInt(tRoads/4), "Roads");
	this.modify("Stability", parseInt(tRoads/8), "Roads");

	// farms
	$('[name="farms"]').setSelect(size);
	var farms = parseInt(this.getChoice("farms", 0));
	if (farms > size)
	{
	    this.setChoice("farms", size);
	    farms = size;
	}
	var farmsProduction = 2*farms;
	var consumption = this.get("Consumption");
	var food = farmsProduction;

	// store this for use externally
	this.modify("foodConsumed", consumption, "Consumption");
	...
	this.modify("farmsProduce", food, "Farms");
...
$.kingdom.LeaderTable = Class.create(
{
	...
	this.addLeader(tbody, 'Royal Enforcer', ['Strength', 'Dexterity'], ['Loyalty'], 'No penalty, but Unrest rate -1 if role filled', function () { });
...
$.kingdom.Building = Class.create(
{
  ...
  buildingData: {
    'Academy': {
      'size': '2x1',
      'cost': 52,
      'halveCost': ["ower", "Library", "Magic Shop"],
      // kingdom
      'minorItems': 3,
      'mediumItems': 2,
      'Economy': 2,
    ...
    'Bardic College': {
      ...
      'Stability': 1,
      'Fame': 1,
      'halvesCost': ["Library", "Museum", "Theater"],
      'minorItems': 2
    },
    'Barracks': {
      'size': '1x1',
    ...
    'Magical Academy': {
      'size': '2x1',
      'cost': 58,
      'halvesCost': ["ower", "Library", "Magic Shop", "Teleport Circle"],
      // settlement
      'cityValue': 2000,
      'Lore': 2,
      'Society': 1,
    ...
    'Palace': {
      'size': '2x2',
      'cost': 108,
      'halveCost': ["Noble Villa", "Mint", "Mansion"],
      // upgradeFrom: "Mansion"
      // kingdom
      'Economy': 2,
      'Loyalty': 6,
    ...
    'Shop': {
      ...
      'cost': 8,
      'adjacentHouses': 1,
      // upgradesTo: "Luxury Store" or "Mansion"
      // settlement
      'cityValue': 500,
      'Productivity': 1,
      // kingdom
    ...
    'Town Hall': {
      'size': '2x1',
      'cost': 22,
      'halveCost': ["Barracks", "Cistern", "Dump", "Watchtower"],
      // settlement
      'Law': 1,
      // kingdom
      'Economy': 1,
    ...
    'University': {
      'size': '2x2',
      'cost': 78,
      'halvesCost': ["Academy", "Bardic College", "Magical Academy", "Museum"],
    ...
    'Waterfront': {
      ...
      'Productivity': 2,
      // kingdom
      'halveKingdom': 'Taxation', // loyalty penalty for taxes
      'minorItems': 2,
      'mediumItems': 2,
      'majorItems': 1,
      'Economy': 4
	...
	if (this.Defense)
	    result += "\nSettlement - Defense " + this.Defense.plus() + ".";
       // settlement
        if (this.Crime)
          result += "\nSettlement - Crime " + this.Crime.plus() + ".";
        if (this.Corruption)
          result += "\nSettlement - Corruption " + this.Corruption.plus() + ".";
        if (this.Law)
          result += "\nSettlement - Law " + this.Law.plus() + ".";
        if (this.Lore)
          result += "\nSettlement - Lore " + this.Lore.plus() + ".";
        if (this.Productivity)
          result += "\nSettlement - Productivity " + this.Productivity.plus() + ".";
        if (this.Society)
          result += "\nSettlement - Society " + this.Society.plus() + ".";
    //
	var items = [];
	if (this.minorItems)
	...
	// settlement 
        if (this.Crime)
	    city.crime += this.Crime;
        if (this.Corruption)
	    city.corruption += this.Corruption;
        if (this.Law)
	    city.law += this.Law;
        if (this.Lore)
	    city.lore += this.Lore;
        if (this.Productivity)
	    city.productivity += this.Productivity;
        if (this.Society)
	    city.society += this.Society;
    //
	var population;
	if (this.size == '1x1')
	    population = 250;
...
$.kingdom.City = Class.create(
{
	...
	// settlement values
        output += "<b>- Crime:</b> " + this.crime + "<br/>";
        output += "<b>- Corruption:</b> " + this.corruption + "<br/>";
        output += "<b>- Law:</b> " + this.law + "<br/>";
        output += "<b>- Lore:</b> " + this.lore + "<br/>";
        output += "<b>- Productivity:</b> " + this.productivity + "<br/>";
        output += "<b>- Society:</b> " + this.society + "<br/>";
        //
...
$.kingdom.Resource = Class.create(
{
    ...
    statList: ['Economy', 'Loyalty', 'Stability', 'IncomeBP'],
